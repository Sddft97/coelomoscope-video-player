...
function randomDate() {
  return randomDateWithStart(1970, 0, 1);
}

function randomDateWithStart(startYear, startMonth, startDay) {
  var maxdaterandom = new Date().getTime();
  // 由于当前环境为北京GMT+8时区，所以与GMT有8个小时的差值
  var mindaterandom = new Date(startYear, startMonth, startDay, 8).getTime();
  var randomdate = getRandom(mindaterandom, maxdaterandom);
  var datestr = new Date(randomdate).toJSON().split('T')[0];
  return datestr;
}
